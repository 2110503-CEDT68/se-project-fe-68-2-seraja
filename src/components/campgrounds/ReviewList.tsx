    ...
    <div className="mt-12">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Based on {reviews.length} stays</p>
        </div>
        ...
        <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Average Rating</span>
          </div>
        </div>
      </div>
          ...
          <Card key={review._id} className="p-6 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-gray-900">{review.user?.name || "Anonymous Guest"}</p>
                  <p className="text-xs text-gray-500">
                    {review.review_createdAt ? new Date(review.review_createdAt).toLocaleDateString() : ""}
                  </p>
                </div>
              </div>
              <div className="flex text-yellow-400">
